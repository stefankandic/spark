...
class ResolveSessionCatalog(val catalogManager: CatalogManager)
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = plan.resolveOperatorsUp {
    ...
    case AlterTableCollation(ResolvedTable(catalog, _, table: V1Table, _), collation)
        if supportsV1Command(catalog) =>
    ...
    case AlterNamespaceCollation(ResolvedV1Database(db), collation) if conf.useV1Command =>
      AlterDatabaseCollationCommand(db, collation)
