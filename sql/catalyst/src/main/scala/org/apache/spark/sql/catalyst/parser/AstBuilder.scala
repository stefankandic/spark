...
/**
  ...
  override def visitCreateNamespace(ctx: CreateNamespaceContext): LogicalPlan = withOrigin(ctx) {
    ...
    checkDuplicateClauses(ctx.collateClause(), "COLLATE", ctx)
    ...
    visitCollateClauseListWithValidation(ctx.collateClause()).foreach {
  ...
  override def visitCreateTableClauses(ctx: CreateTableClausesContext): TableClauses = {
    ...
    val collation = visitCollateClauseListWithValidation(ctx.collateClause())
    val serdeInfo =
