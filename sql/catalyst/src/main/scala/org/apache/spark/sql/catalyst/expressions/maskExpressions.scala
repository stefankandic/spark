...
import org.apache.spark.sql.types.{AbstractDataType, DataType, DefaultStringType}
...
object MaskExpressionBuilder extends ExpressionBuilder {
  override def functionSignature: Option[FunctionSignature] = {
    ...
    val upperCharArg = InputParameter("upperChar",
      Some(Literal.create(Mask.MASKED_UPPERCASE, DefaultStringType)))
    val lowerCharArg = InputParameter("lowerChar",
      Some(Literal.create(Mask.MASKED_LOWERCASE, DefaultStringType)))
    val digitCharArg = InputParameter("digitChar",
      Some(Literal.create(Mask.MASKED_DIGIT, DefaultStringType)))
    val otherCharArg = InputParameter("otherChar",
      Some(Literal.create(Mask.MASKED_IGNORE, DefaultStringType)))
...
case class Mask(
  ...
  def this(input: Expression) =
    this(
      input,
      Literal.create(Mask.MASKED_UPPERCASE, DefaultStringType),
      Literal.create(Mask.MASKED_LOWERCASE, DefaultStringType),
      Literal.create(Mask.MASKED_DIGIT, DefaultStringType),
