...
object PhysicalDataType {
  def apply(dt: DataType): PhysicalDataType = dt match {
    ...
    case VarcharType(_) => PhysicalStringType(DefaultStringType.collationId)
    case CharType(_) => PhysicalStringType(DefaultStringType.collationId)
